...
当前 `v0.1.0` 已发布源码及双平台离线安装资产，完成情况见[版本验收记录](verification.md)。后续 main 文档更新不移动冻结标签、不覆盖资产。
...
2. 继承同提交已有目标系统安装、启动及业务模拟证据。只有具体接口、依赖、打包或安装变化影响这些结果时，先记录改动、风险、范围及停止条件，再做对应补测；不重复未受影响的检查。
...
本版曾使用独立发行辅助分支搬运已经核对的原 CI 产物，未重建包；这是经审核的一次性补齐，不改变上述默认流程，也不允许覆盖已有资产。仅文档变化检查链接、接口说明、版本及公开边界，不触发重复实盘或安装验收。
