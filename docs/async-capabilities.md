...
命令响应与等待底层空闲分别处理：超时错误立即返回页面，不等待 `whenIdle()`。页面出错后重新获取只读快照，显示已持久化的 UNKNOWN，而非保留旧的待执行确认。主进程仍等待底层空闲后才解除变更保护。生命周期等待失败不会覆盖原始命令错误，也不会触发重新执行。

本机真实 Electron 窗口另通过合成超时适配器验证：超时提示与 UNKNOWN 及时可见、执行按钮不可重复点击、复位请求被主进程拒绝，确认底层空闲后可以单独回读为 VERIFIED，提交次数保持 1。此故障注入不代表桌面已接入生产浏览器 Connector。
