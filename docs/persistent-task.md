...
这是同步持久化适配器，不是完整异步 Runtime。离线异步组合、所有权、超时和迟到响应由[异步 Agent](async-task-agent.md)实现；生产连接器的外部操作终止核对不由此适配器提供。`recover()` 只能在确认旧执行者已停止后调用，不能由第二个活跃执行者任意接管。revision 防覆盖不等于平台幂等，也不保证跨系统 exactly-once。

存储内容是本地未签名记录：重放验证一致性，不验证授权来源真实性。自定义 store 必须可信、同步并正确实现原子条件写入；不要接入模型生成的存储/审批函数。

新建桌面价格会话已使用此独立任务存储，库存/活动仍为同步 Mock 会话事务，旧存档保留原路径；见[异步价格会话](async-price-session.md)。SQLite 双连接与组合测试不能证明真实连接器验收。
