...
> 发布状态：本仓库处于 OpsPilot 2.0 Foundation 建设阶段。`dev.4` 预览版支持合成价格调整、库存同步、固定活动报名三条独立模拟闭环，各自需要预览、确认、执行与回读。新建演示使用持久异步价格任务；旧记录不会自动迁移。异常恢复入口、桌面浏览器 Connector、商业仓接入和真实平台验收仍未完成；以下介绍包含计划范围。Open Core 采用 Apache-2.0。

## 快速体验

早期预览版 `0.1.0-dev.4`：

- [Windows x64 安装包](https://github.com/guorunjie/opspilot/releases/download/v0.1.0-dev.4/OpsPilot-Core-Demo-0.1.0-dev.4-win-x64.exe)
- [macOS Apple Silicon 安装包](https://github.com/guorunjie/opspilot/releases/download/v0.1.0-dev.4/OpsPilot-Core-Demo-0.1.0-dev.4-mac-arm64.dmg)
- [发行说明、校验值与已知限制](https://github.com/guorunjie/opspilot/releases/tag/v0.1.0-dev.4)
