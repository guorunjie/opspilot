...
for (const point of ['started', 'target-written']) {
  test(`abrupt child exit at ${point} preserves fences and permits confirmed read-only reconciliation`, async () => {
    ...
    try {
      ...
      assert.equal(before.submissionCount, point === 'started' ? 0 : 1);
      ...
      assert.equal(price.snapshot().task.status, point === 'started' ? 'FAILED' : 'VERIFIED');
